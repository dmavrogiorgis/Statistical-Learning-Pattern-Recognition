...
for i = 1:length(train_C1_indices)
  aRatio_C1(i) = computeAspectRatio(reshape(train_C1_images(i,:,:), 28, 28));
end

for i = 1:length(train_C2_indices)
  aRatio_C2(i) = computeAspectRatio(reshape(train_C2_images(i,:,:), 28, 28));
end
...
rectangle('Position',[min_col-1 min_row-1 width+2 height+2], 'EdgeColor','r', 'LineWidth',3)

subplot(1,2,2);
imagesc(reshape(train_C2_images(500,:,:), 28, 28));
title('Class C2: 500th image');
...
for i=1:length(test_C1_indices)
    test_aRatio_C1(i) = computeAspectRatio(reshape(test_C1_images(i,:,:), 28, 28));
    
    pdf1 = (1/(sqrt(2*pi)*s1))*exp(-(test_aRatio_C1(i)-m1).^2/(2*s1^2));
    pdf2 = (1/(sqrt(2*pi)*s2))*exp(-(test_aRatio_C1(i)-m2).^2/(2*s2^2));
    
    if(pdf1*PC1 > pdf2*PC2)
        BayesClassC1(i) = 1;
    else
        BayesClassC1(i) = 2;
    end
...
for i=1:length(test_C2_indices)
    test_aRatio_C2(i) = computeAspectRatio(reshape(test_C2_images(i,:,:), 28, 28));
  
    pdf1 = (1/(sqrt(2*pi)*s1))*exp(-(test_aRatio_C2(i)-m1).^2/(2*s1^2));
    pdf2 = (1/(sqrt(2*pi)*s2))*exp(-(test_aRatio_C2(i)-m2).^2/(2*s2^2));
